...
train_minority_upsampled = resample(train_minority, 
                                    ...
                                    n_samples=len(train_majority),  # Match the number of majority samples
                                    random_state=42)  # Reproducibility

train_upsampled = pd.concat([train_majority, train_minority_upsampled])

X_train_upsampled = np.stack(train_upsampled['tweet'])
y_train_upsampled = train_upsampled['label'].values
...
history = model.fit(X_train_upsampled, y_train_upsampled, epochs=10, batch_size=64, validation_split=0.2)

y_pred = model.predict(X_test)
y_pred = np.round(y_pred).astype(int)


print("Classification Report (LSTM with Upsampled Data and GloVe Twitter Embeddings):")
